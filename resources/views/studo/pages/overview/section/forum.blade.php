                    ...
                    <div class="col-sm-1">
                        @if($forum->user_id == auth()->user()->id)
                            <div class="dropdown-forum">
                                ...
                                <!-- Form Buat -->
                            <div id="forumFormEdit" style="display: none;">
                                <form action="{{ route('studo.pages.forum.submit', [$class->slug, $chapterId])  }}" method="POST">
                                    @csrf
                                    <div class="row" style="border: 1px solid #E6EBED;border-radius: 5px;padding: 16px;">
                                        <div class="col-sm-1">
...
<script>
$(document).ready(function() {
    $('.dropdown-forum .dropbtn').click(function() {
        $(this).siblings('.dropdown-content-forum').toggleClass('show');
    });

    $(document).click(function(event) {
        if (!$(event.target).closest('.dropdown-forum').length) {
            $('.dropdown-content-forum').removeClass('show');
        }
    });
});
</script>