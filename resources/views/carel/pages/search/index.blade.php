@extends('carel.main')

<style>
    img{
        width:100%;
    }
</style>


@section('content')
<div class="container" style="padding-top:80px;padding-bottom:80px;">
    <div>
        <div>
            <p style="font-weight: 700;font-size: 32px;line-height: 39px;">
                26 hasil untuk “Python”
            </p>
            <p style="font-weight: 400;font-size: 18px;line-height: 22px;">
                Augue purus nisi morbi quam. Tempus varius sed a dictumst pretium cras ornare.
            </p>
            <div class="row" style="display:flex; align-items:center;">
                <div class="col-sm-2">
                    <p style="font-weight: 500;font-size: 16px;line-height: 19px;">
                        Lihat Kategori Lainnya
                    </p>
                </div>
                <div class="col-sm-10">
                    <button class="btn btn-outline-success my-4 my-sm-0 mx-1" style="color:white;background:#063852;" type="submit">
                        <b>
                            Kategori 1
                        </b>
                    </button>
                    <button class="btn btn-outline-success my-4 my-sm-0 mx-1" style="color:#063852; border-color:#063852;background:white" type="submit">
                        <b>
                            Kategori 2
                        </b>
                    </button>
                    <button class="btn btn-outline-success my-4 my-sm-0 mx-1" style="color:#063852; border-color:#063852;background:white" type="submit">
                        <b>
                            Kategori 3
                        </b>
                    </button>
                    <button class="btn btn-outline-success my-4 my-sm-0 mx-1" style="color:#063852; border-color:#063852;background:white" type="submit">
                        <b>
                            Kategori 4
                        </b>
                    </button>
                    <button class="btn btn-outline-success my-4 my-sm-0 mx-1" style="color:#063852; border-color:#063852;background:white" type="submit">
                        <b>
                            Kategori 5
                        </b>
                    </button>
                    <button class="btn btn-outline-success my-4 my-sm-0 mx-1" style="color:#063852; border-color:#063852;background:white" type="submit">
                        <b>
                            Kategori 6
                        </b>
                    </button>
                </div>
            </div>
        </div>

        <div class="row" >
            <div class="col-sm-4"style="margin-top:60px;">
                <img src="data:image/jpeg;base64," alt="">
                <p style="font-weight: 700;font-size: 20px;line-height: 24px;margin-top:24px;margin-bottom:8px;">
                    Pharetra ut morbi tristique sed consectetur neque.
                </p>
                <p style="font-weight: 400;font-size: 16px;line-height: 19px;margin-top:8px;margin-bottom:16px;">
                    Nama Mentor
                </p>
                <p style="font-weight: 700;font-size: 24px;line-height: 29px;color:#063852;margin-top:16px">
                    Rp20.000
                </p>
            </div>
            <div class="col-sm-4"style="margin-top:60px;">
                <img src="data:image/jpeg;base64," alt="">
                <p style="font-weight: 700;font-size: 20px;line-height: 24px;margin-top:24px;margin-bottom:8px;">
                    Pharetra ut morbi tristique sed consectetur neque.
                </p>
                <p style="font-weight: 400;font-size: 16px;line-height: 19px;margin-top:8px;margin-bottom:16px;">
                    Nama Mentor
                </p>
                <p style="font-weight: 700;font-size: 24px;line-height: 29px;color:#063852;margin-top:16px">
                    Rp20.000
                </p>
            </div>
            <div class="col-sm-4"style="margin-top:60px;">
                <img src="data:image/jpeg;base64," alt="">
                <p style="font-weight: 700;font-size: 20px;line-height: 24px;margin-top:24px;margin-bottom:8px;">
                    Pharetra ut morbi tristique sed consectetur neque.
                </p>
                <p style="font-weight: 400;font-size: 16px;line-height: 19px;margin-top:8px;margin-bottom:16px;">
                    Nama Mentor
                </p>
                <p style="font-weight: 700;font-size: 24px;line-height: 29px;color:#063852;margin-top:16px">
                    Rp20.000
                </p>
            </div>
            <div class="col-sm-4"style="margin-top:60px;">
                <img src="data:image/jpeg;base64," alt="">
                <p style="font-weight: 700;font-size: 20px;line-height: 24px;margin-top:24px;margin-bottom:8px;">
                    Pharetra ut morbi tristique sed consectetur neque.
                </p>
                <p style="font-weight: 400;font-size: 16px;line-height: 19px;margin-top:8px;margin-bottom:16px;">
                    Nama Mentor
                </p>
                <p style="font-weight: 700;font-size: 24px;line-height: 29px;color:#063852;margin-top:16px">
                    Rp20.000
                </p>
            </div>
            <div class="col-sm-4"style="margin-top:60px;">
                <img src="data:image/jpeg;base64," alt="">
                <p style="font-weight: 700;font-size: 20px;line-height: 24px;margin-top:24px;margin-bottom:8px;">
                    Pharetra ut morbi tristique sed consectetur neque.
                </p>
                <p style="font-weight: 400;font-size: 16px;line-height: 19px;margin-top:8px;margin-bottom:16px;">
                    Nama Mentor
                </p>
                <p style="font-weight: 700;font-size: 24px;line-height: 29px;color:#063852;margin-top:16px">
                    Rp20.000
                </p>
            </div>
            <div class="col-sm-4"style="margin-top:60px;">
                <img src="data:image/jpeg;base64," alt="">
                <p style="font-weight: 700;font-size: 20px;line-height: 24px;margin-top:24px;margin-bottom:8px;">
                    Pharetra ut morbi tristique sed consectetur neque.
                </p>
                <p style="font-weight: 400;font-size: 16px;line-height: 19px;margin-top:8px;margin-bottom:16px;">
                    Nama Mentor
                </p>
                <p style="font-weight: 700;font-size: 24px;line-height: 29px;color:#063852;margin-top:16px">
                    Rp20.000
                </p>
            </div>

        </div>
    </div>
</div>
@endsection